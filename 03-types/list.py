...
hundreds = [num for num in range(1 ,2000 + 1) if not num % 200]
...
ascii1 = [random.choice(string.ascii_letters).upper() for i in range(50)]
...
combine = list(zip(hundreds, ascii1))
...
persons += (('Petr', 30, 'muž'), ('Tomáš', 38, 'muž'), ('Karina', 25, 'žena'), ('Alice', 28, 'žena'))
print(f'Aktualizovaný seznam person\n\t{persons}')
...
women = list(filter(lambda item: item[2] == 'žena', persons)); print('\nSeznam žen\n----------------')
for i in range(len(women)): print(f'{women[i][0]}\n{"-" * len(women[i][0])}')

women = [item[0] for item in persons if item[2] == 'žena']; print('\nSeznam žen\n----------------')
for i in women: print(f'{i}\n{"-" * len(i)}')
...
ipeople = list(filter(lambda item: item[0].lower().find("i") >= 0, persons)); ipeople.sort(key=sort_item, reverse=True)
print('\nCSV struktura i-lidí\n--------------------')
for i in range(len(ipeople)): print(f'{i};{ipeople[i][0]};{ipeople[i][1]};{ipeople[i][2]}')

ipeople = [item for item in persons if item[0].lower().find("i") >=0]; ipeople.sort(key=sort_item, reverse=True)
print('\nCSV struktura i-lidí\n--------------------')
for i in range(len(ipeople)): print(f'{i};{ipeople[i][0]};{ipeople[i][1]};{ipeople[i][2]}')