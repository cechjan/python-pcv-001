...
now = datetime.now()
print('Aktuální datum a čas je: ', now)
...
for i in range(5):
    year += 1
    rdelta = relativedelta(easter(year), now)
    print("Datum Velikonoční neděle v následujících letech: %s" % (now + rdelta))
...
print("Nejbližší rok, v němž bude Štědrý den v neděli: %s" % year)