...
def text_to_var_name(souslovi, znak, lower_case):
    print(unidecode.unidecode(souslovi.lower().replace(' ', znak)) if lower_case else unidecode.unidecode(souslovi.title().replace(' ', znak)))

#   a)
text_to_var_name(text, '_', True)

#   b)
text_to_var_name(text, '', False)


#   3
import random, string

def generate_password(quantity):
...
generate_password(input('Zadejte počet hesel: '))